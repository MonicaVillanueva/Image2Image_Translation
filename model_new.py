...
class ResidualBlock():
	...
	def __init__(self, dim_in=256, dim_out=256):
		self.Y1 = tf.layers.Conv2D(filters=dim_out, kernel_size=[3, 3], padding="same", activation=tf.nn.relu,use_bias=True, strides=1)
		self.Y1_norm = tf.layers.BatchNormalization()
		self.Y2 = tf.layers.Conv2D(filters=dim_out, kernel_size=[3, 3], padding="same", activation=tf.nn.relu,use_bias=True, strides=1)
...
class Generator():
	...
	def __init__(self, imgDim=(128,128), numClass=5):
		with tf.variable_scope("Generator") as self.Gscope:
			...
			self.downSampling1 = tf.layers.Conv2D(filters=64,kernel_size=[7, 7],padding="same",activation=tf.nn.relu, use_bias=True, strides=1)
			self.downSampling2 = tf.layers.Conv2D(filters=128,kernel_size=[4, 4],padding="same",activation=tf.nn.relu, use_bias=True, strides=2)
			self.downSampling3 = tf.layers.Conv2D(filters=256,kernel_size=[4, 4],padding="same",activation=tf.nn.relu, use_bias=True, strides=2)
			...
			self.Y_upSampling1 = tf.layers.Conv2DTranspose(filters=128,kernel_size=[4, 4],padding="same",activation=tf.nn.relu, use_bias=True, strides=2)
			self.Y_upSampling1_norm = tf.layers.BatchNormalization()
			self.Y_upSampling2 = tf.layers.Conv2DTranspose(filters=64,kernel_size=[4, 4],padding="same",activation=tf.nn.relu, use_bias=True, strides=2)
			self.Y_upSampling2_norm = tf.layers.BatchNormalization()

			self.fakeGeneration = tf.layers.Conv2D(filters=3,kernel_size=[7, 7],padding="same",activation=tf.nn.tanh, use_bias=True, strides=1) #FIXME PADDING P3 IN TF...?
...
class Discriminator():
	...
	def __init__(self, imageSize=128, convDim=64, numClass=5):
		with tf.variable_scope("Discriminator") as self.Dscope:
			...
			self.OutputLayerSrc = tf.layers.Conv2D(filters=1,kernel_size=[3, 3],padding="same",activation=None, use_bias=True, strides=1)

			self.OutputLayerCls = tf.layers.Conv2D(filters=numClass,kernel_size=[imageSize/64, imageSize/64],padding="valid",activation=None, use_bias=True, strides=1)

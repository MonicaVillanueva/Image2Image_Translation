...
class ResidualBlock():
	...
	def forward(self, x):
		Y1 = tf.nn.conv2d(x, self.residualBlock1_W, strides=[1, 1, 1, 1],
						  padding='SAME') + self.residualBlock1_b
		Y1_norm = tf.contrib.layers.batch_norm(Y1)

		Y1_relu = tf.nn.relu(Y1_norm)

		Y2 = tf.nn.conv2d(Y1_relu, self.residualBlock2_W, strides=[1, 1, 1, 1],
						  padding='SAME') + self.residualBlock2_b
